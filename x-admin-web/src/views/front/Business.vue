        ...
        <div style="margin-top: 20px">
          <el-row>
            <el-col :span="5" style="margin-bottom: 20px" v-for="item in goodsData">
              <img :src="item.img" alt="" style="width: 100%; height: 150px; border-radius: 10px; border: #cccccc 1px solid" @click="navTo('/detail?id=' + item.id)">
...
export default {
  ...
  methods: {
    async loadBusiness() {
      const res = await userApi.selectById(this.businessId)
      if(res.code === 20000){
    ...
    async loadGoods() {
      const res = await goodsApi.selectByBusinessId(this.businessId)
      console.log("res: ",res)
      if(res.code === 20000){
