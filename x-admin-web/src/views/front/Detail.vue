      ...
      <div style="padding: 15px 20px">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            ...
            <div style="color: black; margin-top: 15px;;">原价：<span style="font-size: 20px">{{goodsData.price}} / {{goodsData.unit}}</span></div>
            <div v-if="goodsData.activityStatus === '1' && goodsData.activityDiscount < 1" style="color: red; margin-top: 15px">折扣价：
              <span  style="font-size: 30px">{{goodsData.price*goodsData.activityDiscount}} / {{goodsData.unit}}</span>
            </div>
...
export default {
  ...
  methods: {
    async loadGoods() {
      const res = await goodsApi.selectById(this.goodsId)
      console.log(res)
      if (res.code === 20000) {
    ...
    async loadComments() {
      const res = await goodsApi.selectCommentByGoodsId(this.goodsId)
      // console.log(res)
      if (res.code === 20000) {
    ...
    async collect() {
      ...
      const res = await utilsApi.addUpdate('/collect/add','post',data)
      if(res.code === 20000){
    ...
    async addCart() {
      ...
      const res = await utilsApi.addUpdate('/cart/add','post',data)
      if(res.code === 20000){
        this.$message.success(res.message)
      }else{
        this.$message.error(res.message)
