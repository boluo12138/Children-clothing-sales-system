<template>
  <div>
    <div class="search">
      <el-input placeholder="请输入评论内容" style="width: 200px" v-model="selectForm.content"></el-input>
...
export default {
  ...
  methods: {
    ...
    del(id) {   // 单个删除
      this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(async () => {
        const res = await utilsApi.deleteById("/comment/delete/"+id)
          if(res.code === 20000){
              // 表示操作成功
    ...
    delBatch() {   // 批量删除
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', {type: "warning"}).then(async () => {
        const res = await utilsApi.deleteByList("/comment/delete/batch",this.ids)
          if(res.code === 20000){
            // 表示操作成功
